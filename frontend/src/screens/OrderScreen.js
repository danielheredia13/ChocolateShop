...
if (order) {
  ...
  totalPrice = order.totalPrice
    ...
    if (order) {
        order.itemsPrice =addDesimals(Number(orderItems.reduce((acc,cur) => acc + cur.qty * cur.price, 0)))
    }
  ...
  return loading ? <Loader /> : error ? <Message variant="danger" text={error} />
    : <>
    <h2>Order { order && order._id }</h2>
     <Row>
        ...
        <ListGroup variant='flush'>
            <ListGroup.Item>
                <h4>Shipping</h4>
                <p><strong>User: </strong>{order && order.user.name}</p>
                ...
                {order && order.isDelivered ? <Message variant="success" text={`Delivered at ${order.deliveredAt}`} />
                : <Message variant="danger" text="Not Delivered" />}
            </ListGroup.Item>
            <ListGroup.Item>
                <h4>Payment Method</h4>
                <p><strong>Method: </strong>{ paymentMethod }</p>
                {order && order.isPaid ? <Message variant="success" text={`Paid at ${order.paidAt}`} />
            ...
            <Card className='order-summary'>
                <ListGroup variant='flush'>
                   ...
                   }
                   {
                    ...
                    && <ListGroup.Item>
                        <Button onClick={() => deliverHandler(order._id)} type='button' className='btn rounded' variant='dark'>Set as Delivered</Button>
                    </ListGroup.Item>
                   }
                </ListGroup>
            </Card>
